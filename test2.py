...
def get_elevation(coord, corner_coord, grid):
    ...
    ind_lon = int(3600 * abs(corner_lon - lon))
    ind_lat = int(3600 * abs(corner_lat - lat))

    return grid[ind_lon][ind_lat] if grid[ind_lon][ind_lat] > 0 else 3000


def create_plot_data(coord, size, corner_coord, grid):

    # creating arrays with coordinates
    coords_x = np.array([x + 3 * 0.0002777777777777778 * i 
                for i in range(-size, size + 1)] * (2 * size + 1))
    coords_y = np.array([y + 3 * 0.0002777777777777778 * i for i in range(-size, size + 1) 
...
if __name__ == "__main__":
    x, y = [ 45.831513, 6.864683 ]
    size = 100
    grid = read_data('geofiles/n45_e006_1arc_v3.tif')
    plot_data = create_plot_data((x, y), size, (45, 6), grid)
